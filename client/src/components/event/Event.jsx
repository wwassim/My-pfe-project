...
const Event = ({status,events}) => {
  ...
  //  console.log item.user.followers
  //  console.log(item.user.followings)  
    if (item.user!== null){
      return(<div key={item._id} class="!z-5 shadow-3xl shadow-shadow-500 3xl:p-![18px] undefined relative flex flex w-full max-w-[300px] flex-col flex-col rounded-[20px] bg-white bg-white bg-clip-border !p-4">
        ...
        <div className="relative w-full h-[200px]">
          ...
          <button class="absolute top-3 right-3 flex items-center justify-center rounded-full bg-white p-2 hover:cursor-pointer">
            <div class="flex h-full w-full items-center justify-center rounded-full text-xl ">
              {liked ?<FavoriteIcon sx={{ color: pink[500] }} onClick={() => removeWishlishHandler(item)} />:<FavoriteBorderIcon onClick={() => addToWishHandler(item)}/>}
           </div>
          </button>
